  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ fontWeight: 800, fontSize: 20, color: "#FF6B00", display: "flex", alignItems: "center", gap: 6 }}>
          🔮 AsthaAI
        </div>
      ...
      <div style={{ padding: "0 0 80px" }}>
        ...
        <div style={{ padding: "14px 14px 0", background: "#fff", borderBottom: "1px solid #ebebeb" }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: "#333", marginBottom: 10 }}>🟢 Live Astrologers</div>
          <div style={{ display: "flex", gap: 16, overflowX: "auto", paddingBottom: 14, scrollbarWidth: "none" }}>
            {ASTROLOGERS.filter((a) => a.available).slice(0, 5).map((a) => (
              ...
              >
                ...
                <div style={{ fontSize: 10, color: "#555", textAlign: "center", lineHeight: 1.2, maxWidth: 70, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
        ...
        <div style={{ padding: "10px 14px" }}>
          <div style={{ background: "linear-gradient(135deg, #FF6B00, #FF8F3F)", borderRadius: 12, padding: "14px 16px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div>
              <div style={{ color: "#fff", fontWeight: 800, fontSize: 14, lineHeight: 1.3 }}>Recharge with ₹500 &</div>
              <div style={{ color: "#FFE0B2", fontWeight: 800, fontSize: 16 }}>Get ₹300 EXTRA!</div>
            </div>
        ...
        <div style={{ padding: "4px 14px 10px" }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: "#888", marginBottom: 8 }}>Kisliye help chahiye?</div>
          <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 4, scrollbarWidth: "none" }}>
            {INTENT_CHIPS.map((chip) => (
              <button
                key={chip.label}
                onClick={() => setActiveChip(activeChip === chip.filter ? null : chip.filter)}
                style={{
        ...
        <div style={{ padding: "0 14px" }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: "#333", marginBottom: 10 }}>Hamare Astrologers</div>
          {filtered.map((a) => (
            <AstroCard
              key={a.id}
              astrologer={a}
