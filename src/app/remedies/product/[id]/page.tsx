...
export default function ProductDetail() {
  ...
  const isFemale = recommender.name.includes("Devi") || recommender.name.includes("Priya") || recommender.name.includes("Sana") || recommender.name.includes("Meera") || recommender.name.includes("Kavita") || recommender.name.includes("Divya");
    ...
    >
      ...
      <div style={{ position: "sticky", top: 0, zIndex: 50, background: "#fff", borderBottom: "1px solid #ebebeb", padding: "12px 16px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <span style={{ fontWeight: 700, fontSize: 16, color: "#333" }}>Product Details</span>
        </div>
      ...
      <div style={{ padding: "0 0 80px" }}>
        ...
        <div style={{ padding: "12px 14px" }}>
          <div style={{ background: "#FFF8F2", borderLeft: "4px solid #FF6B00", borderRadius: "0 10px 10px 0", padding: 14 }}>
            <div style={{ fontWeight: 700, fontSize: 14, color: "#FF6B00", marginBottom: 10 }}>🎯 Yeh aapke liye hai agar...</div>
        ...
        <div style={{ padding: "0 14px 12px" }}>
          <div style={{ background: "#F1F8E9", borderLeft: "4px solid #4CAF50", borderRadius: "0 10px 10px 0", padding: 14 }}>
            <div style={{ fontWeight: 700, fontSize: 14, color: "#2E7D32", marginBottom: 10 }}>📈 Kya expect karein?</div>
            {product.timeline.map((t, i) => (
              <div key={i} style={{ display: "flex", gap: 10, marginBottom: 8, alignItems: "flex-start" }}>
                <span style={{ background: "#C8E6C9", color: "#2E7D32", fontSize: 10, fontWeight: 700, padding: "3px 8px", borderRadius: 6, whiteSpace: "nowrap", minWidth: 55, textAlign: "center" }}>{t.week}</span>
        ...
        <div style={{ background: "#fff", padding: "14px 16px", borderTop: "1px solid #ebebeb", borderBottom: "1px solid #ebebeb" }}>
          <div style={{ fontWeight: 700, fontSize: 14, color: "#333", marginBottom: 8 }}>About {product.name}</div>
        ...
        {/* Reviews - with conversion proof */}
        <div style={{ background: "#fff", padding: "14px 16px", borderBottom: "1px solid #ebebeb", marginTop: 1 }}>
          <div style={{ fontWeight: 700, fontSize: 14, color: "#333", marginBottom: 12 }}>🗣️ Jinke kaam aaya</div>
          {product.reviews.map((review, i) => (
            <div key={i} style={{ background: "#FAFAFA", border: "1px solid #f0f0f0", borderRadius: 10, padding: 12, marginBottom: 10 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    <span style={{ background: "#E8F5E9", color: "#2E7D32", fontSize: 9, fontWeight: 700, padding: "1px 6px", borderRadius: 6 }}>✓ Verified Purchase</span>
                  </div>
                </div>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                {i === 0 && <span style={{ fontSize: 11, color: "#FF6B00", fontWeight: 600, cursor: "pointer" }}>📹 Video review available</span>}
              </div>
        ...
        <div style={{ background: "#fff", padding: "14px 16px", borderBottom: "1px solid #ebebeb", marginTop: 1 }}>
          <div style={{ fontWeight: 700, fontSize: 14, color: "#333", marginBottom: 12 }}>👨‍🔬 Astrologer bhi recommend karte hain</div>
          <div style={{ display: "flex", alignItems: "center", gap: 12, background: "#FFF8F2", border: "1px solid #FFE4C8", borderRadius: 12, padding: 12 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: "#888", lineHeight: 1.4 }}>
                &ldquo;Yeh product {product.category.toLowerCase()} ke liye recommend kart{isFemale ? "i" : "a"} hoon&rdquo;
              </div>
            </div>
          </div>
          <button onClick={() => router.push(`/astrologer/${recommender.id}`)} style={{ width: "100%", background: "transparent", border: "1.5px solid #FF6B00", color: "#FF6B00", borderRadius: 10, padding: "10px 0", fontWeight: 700, fontSize: 13, cursor: "pointer", marginTop: 10 }}>
      ...
      <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 420, display: "flex", zIndex: 50 }}>
        ...
        <button style={{ flex: 1, background: "#E65100", color: "#fff", border: "none", fontWeight: 700, fontSize: 16, cursor: "pointer", padding: "14px 0" }}>CONTINUE →</button>
      </div>
    </div>
  );
}
